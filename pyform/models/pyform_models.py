            ...
            if self.model_json_schema().get('$defs'):
                ...
                for key2, value2 in self.model_json_schema().get('$defs').items():
                    yield f"""<div class="collapse collapse-arrow join-item border-base-300 border">
                        ...
                        <div class="collapse-title font-semibold"> <div class="badge badge-info">{key2}</div></div>
                    ...
                    for key3, value3 in value2.get('properties').items():
                        ...
                        if value3.get('type') == 'number':
                            if values:
                                ...
                                yield f"""<input  class="input is_primary" type="number" step="0.001" name="{key3}" id="{key3}" placeholder="{value3.get('title')}" /></label>"""
                                                
                            if errors and form.get('fields', {}).get(key3, {}).get('error'):
                                yield f""" <div class="text-xs text-red-500 font-semibold">{form.get('fields', {}).get(key3, {}).get('error')}</div></fieldset>"""
                            else:
                                yield """</fieldset>"""
                        # do further checks for file upload field, radio buttons, checkboxes,select fields etc...
                        else:                            
                            if values:
                                ...
                                yield f""" <input  class="input is_primary" type="{value3.get('type')}" name="{key3}" id="{key3}" placeholder="{value3.get('title')}" /></label>"""
